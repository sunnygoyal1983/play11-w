// Simple script to import data from SportMonk API
...
async function fetchUpcomingMatches(page = 1, perPage = 10) {
  try {
    const response = await sportmonkApi.get('/fixtures', {
      params: {
...
async function fetchTeamPlayers(teamId) {
  try {
    const response = await sportmonkApi.get(`/teams/${teamId}`, {
      params: {
        include: 'squad,squad.player'
      }
    });
    return response.data;
  } catch (error) {
...
async function fetchPlayerDetails(playerId) {
  try {
    const response = await sportmonkApi.get(`/players/${playerId}`, {
      params: {
        include: 'career,country,position'
      }
    });
    return response.data;
  } catch (error) {
...
async function importUpcomingMatches() {
  try {
    ...
    const upcomingMatchesData = await fetchUpcomingMatches(1, 20);
  ...
  try {
    ...
    for (const match of matches) {
      ...
      for (const teamId of teamIds) {
        ...
        const teamData = await fetchTeamPlayers(parseInt(teamId));
        ...
        for (const squadMember of team.squad) {
          ...
          const playerDetailsData = await fetchPlayerDetails(player.id);
