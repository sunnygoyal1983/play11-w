...
export default function AdminMatchTeams() {
  ...
  const filteredTeams = teams.filter((team: any) => {
    ...
    const matchesFormatFilter = formatFilter === 'all' || team.format === formatFilter;
  ...
  const handleUpdateTeam = async (teamId: string, name: string, logo: string) => {
    try {
      const response = await fetch('/api/match-teams', {
        method: 'PATCH',
        headers: {
                      ...
                      <div className="flex items-center">
                        {team.logo ? (
                          ...
                          <div className="flex-shrink-0 h-10 w-10 mr-3 bg-gray-200 rounded-full flex items-center justify-center">
                            <span className="text-gray-500 font-semibold">{team.name.substring(0, 2).toUpperCase()}</span>
                          </div>
                        )}
